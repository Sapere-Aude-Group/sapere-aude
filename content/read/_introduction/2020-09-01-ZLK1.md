...
### 5. 提交内容：原始论文、支持资料、读书报告

- 每个人在代码库 [sapere-aude](https://github.com/likanzhan/sapere-aude) 的 `content/read/` 目录下都有一个以你自己名字命名的文件夹，如 `_introduction`。你需要把读书报告上传到这里，文件夹下还有两个子文件夹 `Source_Files` 和 `Supporting_Information`，你需要把原始论文和支持资料（读书报告中用到的图和表）分别传到这两个文件夹里。
...
- 每个 `md` 文档必须有一个文档表头（front matter）。表头的开始和结尾由 “`---`”。
- 该符号不能缺失，且必须独占一行。理论上来讲，表头可以包含很多内容。但目前我们用到的是下面一些：
```
---
